...
var icon_cities =
  'https://clientes.mi-martinez.com/sabi/wp-content/uploads/2020/08/icon-point-purple.png'
var icon_type1 =
  'https://clientes.mi-martinez.com/sabi/wp-content/uploads/2020/08/icon-1.png'
var icon_type2 =
  'https://clientes.mi-martinez.com/sabi/wp-content/uploads/2020/08/icon-2.png'
var icon_matpel =
  'https://clientes.mi-martinez.com/sabi/wp-content/uploads/2020/08/icon-3.png'

var text_type1 = 'Tipo 1'
var text_type2 = 'Tipo 2'
var text_matpel = 'Matpel'
...
function initMap() {
  ...
  var rendererEast = new google.maps.DirectionsRenderer({
    preserveViewport: true,
  })
  let markersEast = {points: [], cities: [], type1: [], type2: [], matpel: []}

  var rendererCenter = new google.maps.DirectionsRenderer({
    preserveViewport: true,
  })
  let markersCenter = {points: [], cities: [], type1: [], type2: [], matpel: []}

  var rendererNort = new google.maps.DirectionsRenderer({
    preserveViewport: true,
  })
  let markersNort = {points: [], cities: [], type1: [], type2: [], matpel: []}

  var rendererSouth = new google.maps.DirectionsRenderer({
    preserveViewport: true,
  })
...
function selectedWaypoints(zone, markers, map) {
  ...
  Object.keys(zones[zone].points).map((key) => {
    ...
    array.forEach((item) => {
      ...
      if (key !== 'cities') {
        var content =
          '<div class="popup-marker" id="content">' +
          `<h5>${item.name}</h5>` +
          `<p>${eval('text_' + key)}</p>` +
          '</div>'
